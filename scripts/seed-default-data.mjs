...
import { coursesTable, semestersTable, subjectsTable, studyMaterialsTable } from '../config/schema.jsx';
...
async function seedDefaultData() {
    try {
        ...
        // Summary
        const totalSubjects = await db.select().from(subjectsTable);
        const totalMaterials = await db.select().from(studyMaterialsTable);

        console.log('\n Default data seeding completed successfully!');
        console.log(`📊 Summary:`);
        console.log(`    Total Subjects: ${totalSubjects.length}`);
        console.log(`   📄 Total Study Materials: ${totalMaterials.length}`);
